...
import { IosDriver } from 'kuben-appium-ios-driver';
import { AndroidUiautomator2Driver } from 'appium-uiautomator2-driver';
import { SelendroidDriver } from 'appium-selendroid-driver';
import { XCUITestDriver } from 'kuben-appium-xcuitest-driver';
...
import { simlatorclient } from 'kuben-appium-ios-simswitch';
...
const DRIVER_MAP = {
  ...
  XCUITestDriver: {
    ...
    version: getPackageVersion('kuben-appium-xcuitest-driver'),
  },
  // YouiEngineDriver: {
  ...
  IosDriver: {
    ...
    version: getPackageVersion('kuben-appium-ios-driver'),
  },
  WindowsDriver: {
...
class AppiumDriver extends BaseDriver {
   ...
   * @param  
   */
  async sysKillSimulator(automationName='') {
    log.debug('new test start!! clear simulator first!!')
    //var simctl = require('appium-ios-test')
    //console.log(simctl.simlatorctlserver)
    //console.log(simlatorclient)
    if(automationName == null || automationName == ''){
      automationName = "iosAutomation";
    }
    log.debug('test type name:'+automationName);
    await simlatorclient.startSimulatorTestClient(automationName)
    setTimeout(function () {
  ...
  async createSession (jsonwpCaps, reqCaps, w3cCapabilities) {
    ...
    try {
      ...
      await this.sysKillSimulator(desiredCaps.automationName);
